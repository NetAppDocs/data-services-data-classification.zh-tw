...
如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/data-services-data-classification/task-health-monitor.html["監控資料分類的運作狀況"^]。
...
此 BlueXP 分類版本包含可最佳化資源使用率的程式碼變更。
...
Data Classification 支援對 Map & Classify 掃描以及 Map-only 掃描進行優先排序的功能，讓您可以選擇優先完成哪些掃描。Map & Classify 掃描的優先排序在掃描開始前和進行中均受支援。如果您選擇在掃描進行過程中設定優先順序，則對應和分類掃描都會被優先處理。
...
Data Classification 支援將分類事件（掃描開始時和結束時）傳送至 link:https://docs.netapp.com/us-en/console-setup-admin/task-monitor-cm-operations.html#audit-user-activity-from-the-bluexp-timeline["NetApp Console 稽核頁面"^]。
...
* 啟動審計日誌收集的選項已停用。
* 在目錄調查期間，無法計算目錄中個人識別資訊 (PII) 資料的數量。請參閱 link:task-investigate-data.html["調查組織中儲存的數據"]。
...
Red Hat Enterprise Linux 8 和 9 版本不支援 Docker 引擎，而 Docker 引擎之前是 BlueXP classification 安裝所必需的。現在，BlueXP classification 支援在 RHEL 9.0、9.1 和 9.2 上安裝，但需要使用 Podman 4 或更高版本作為容器基礎架構。如果您的環境需要使用最新版本的 RHEL，現在您可以使用 Podman 安裝 BlueXP classification（1.26 或更高版本）。

目前，在使用 RHEL 9.x 時，Data Classification 既不支援暗站安裝，也不支援分散式掃描環境（使用主掃描節點和遠端掃描節點）。
...
「存取的使用者數」篩選器現在支援儲存在 SharePoint Online 儲存庫中的檔案。此前，僅支援 CIFS 共用上的檔案。目前，此篩選器暫不統計非 Active Directory 類別的 SharePoint 使用者群組。
