...
---
= 使用 NetApp Data Classification 檢視有關組織私人資料的合規性詳細資訊
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: 只有執行完整分類掃描才能獲得檔案層級法規遵循詳細資料。僅對應掃描無法產生檔案層級詳細資料。
...
+
image:screenshot_compliance_investigation_page.png["選取「調查結果」後，在檔案中找到的詳細資訊螢幕截圖。"]

+
image:screenshot_compliance_investigation_page_directory.png["選取「調查結果」後，在目錄中找到的詳細資訊螢幕擷取畫面。"]
...
. 您可以搜尋、排序、展開特定檔案的詳細資訊、選擇 *Investigate Results* 以查看遮罩資訊，或下載檔案清單來調查資料。
