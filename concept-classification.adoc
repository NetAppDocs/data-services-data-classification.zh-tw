...
[role="lead"]
NetApp Data Classification 是 NetApp Console 中的資料管制服務，可掃描您的本地和雲端資料來源，對資料進行對應和分類，並識別私密資訊。它有助於降低安全性和循規風險、減少儲存設備成本，並支援資料移轉專案。


IMPORTANT: 從 1.31 版開始，Data Classification 已作為 NetApp Console 的核心功能提供。無需額外付費。無需 Data Classification 授權或訂閱。+ 如果您之前使用的是舊版 1.30 或更早版本，該版本仍可在您的訂閱到期前繼續使用。
...
NetApp Console 提供企業級本機和雲端環境中 NetApp 儲存設備和資料服務的集中管理。存取和使用 NetApp 資料服務需要透過 Console。作為管理介面，它可讓您透過單一介面管理多個儲存資源。Console 系統管理員可以控制企業內所有系統對儲存設備和服務的存取。
...
.優化儲存使用情況
資料分類提供了一系列工具，可協助降低儲存設備總體擁有成本（TCO）。您可以使用資料分類來：

* 透過識別重複或與業務無關的資料來提高儲存效率。
* 透過識別可以分層到較便宜的物件儲存的非活動資料來節省儲存成本。 https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/concept-data-tiering.html["了解有關Cloud Volumes ONTAP系統分層的更多信息"^] 。 https://docs.netapp.com/us-en/data-services-cloud-tiering/concept-cloud-tiering.html["了解有關本地ONTAP系統分層的更多信息"^] 。
...
資料傳輸成本取決於您的設定。如果 NetApp Data Classification 執行個體和資料來源位於相同可用度區和區域，則不會產生資料傳輸成本。但如果資料來源（例如 Cloud Volumes ONTAP 系統）位於不同的可用度區或區域，則您的雲端供應商會向您收取資料傳輸成本。如需更多詳情，請參閱以下連結：
...
* 在 AWS 中，Data Classification 運行在 https://aws.amazon.com/ec2/instance-types/m6i/["m6i.4xlarge 實例"^]上，配備 500 GiB GP2 磁碟。作業系統映像為 Amazon Linux 2。
...
* **僅對應掃描** 僅提供資料的高階概覽，並且僅針對選定的資料來源執行。由於僅對應掃描無需存取檔案來查看內部資料，因此比完整掃描耗時更短。您可以先執行此操作以確定研究領域，然後再對這些領域執行完整掃描。
* **完整掃描**可對您的資料進行深度掃描。完整掃描包括僅地圖掃描和對檔案內資料進行分類。


如需僅對應掃描與完整掃描之間差異的詳細資訊，請參閱 link:task-scanning-overview.html["映射和分類掃描之間有什麼區別？"]。
...
* *類別*：資料分類將掃描的資料分為不同類型的類別。類別是基於 AI 對每個文件的內容和元資料的分析的主題。link:task-controlling-governance-data.html["了解有關類別的更多信息"^]。
