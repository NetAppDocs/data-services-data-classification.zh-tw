...
[role="lead"]
使用 NetApp Data Classification REST API 可以存取與 Web UI 中相同的功能。
...
** 資料以 JSON 格式匯出（易於解析並推送至第三方應用程式，例如 Splunk）。


* 使用“AND”和“OR”語句建立查詢，包括和排除資訊等等。
+
例如，您可以尋找不含特定個人識別資訊 (PII) 的檔案（此功能在使用者介面中不提供）。您也可以排除匯出作業中的特定欄位。
...
要存取 Swagger，您需要 NetApp Data Classification 執行個體的 IP 位址。對於雲端部署，請使用公有 IP 位址。然後，您需要存取以下端點：
...
.API 請求
您需要取得系統的所有相關欄位和選項，才能在「調查」標籤中檢視所有篩選條件。
