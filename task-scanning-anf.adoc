...
.結果
資料分類功能開始掃描您在系統中選擇的磁碟區。資料分類功能完成初始掃描後，結果會立即顯示在合規性儀表板中。所需時間取決於資料量，可能需要幾分鐘或幾小時。您可以透過導覽至 *Configuration* 選單中的系統概覽頁面來追蹤初始掃描的進度。資料分類功能會為每次掃描顯示進度條。您可以將滑鼠懸停在進度條上，查看已掃描檔案數佔磁碟區中檔案總數的比例。
