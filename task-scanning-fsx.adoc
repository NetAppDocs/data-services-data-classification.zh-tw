...
[NOTE]
====
* 預設情況下，如果 Data Classification 在 CIFS 中沒有寫入屬性權限，或在 NFS 中沒有寫入權限，系統將不會掃描磁碟區中的檔案，因為 Data Classification 無法將「上次存取時間」還原為原始時間戳記。如果您不介意上次存取時間是否重設，請選取 *Or select scanning type for each volume*。在產生的頁面中，有一個可啟用的設定，讓 Data Classification 無論權限如何都會掃描磁碟區。
