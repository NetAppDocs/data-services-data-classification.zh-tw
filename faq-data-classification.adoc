...
掃描效能會因網路頻寬和環境中的平均檔案大小而異。它還取決於主機系統的大小和位置（即，是在雲端還是在內部部署）。如需詳細資訊，請參閱 link:concept-classification.html["資料分類實例"] 和 link:task-deploy-overview.html["部署資料分類"]。

首次新增資料來源時，您也可以選擇僅執行僅對應掃描或完整掃描。由於無需存取檔案即可查看其中的資料，因此可以非常快速地對資料來源進行對應。link:task-scanning-overview.html["了解對應掃描與分類掃描之間的差異"]。
...
* *Data Classification 深度掃描（完整掃描）*：Data Classification 使用標準協定和唯讀權限掃描您環境中的所有資料。系統會開啟選定的檔案並進行掃描，以尋找敏感的業務相關資料、私人資訊以及與勒索軟體相關的問題。
+
