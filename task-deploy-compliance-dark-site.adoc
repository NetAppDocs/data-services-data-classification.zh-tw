...
---
= 在私有模式下於 Linux 主機上安裝 NetApp Data Classification
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用「私有模式」在無法存取網際網路的本機站台（也稱為離線安裝或實體隔離環境）的 Linux 主機上安裝 NetApp Data Classification。此部署使用安裝指令碼，並將 Data Classification 執行個體與 NetApp Console SaaS 階層中斷連線，讓您在隔離的環境中掃描本機資料來源。
...
Data Classification 安裝指令碼會檢查系統和環境是否符合所需的先決條件。如果符合，則開始安裝。如果您希望在不執行 Data Classification 安裝的情況下驗證先決條件，可以下載一個分隔的軟體套件，該套件僅用於測試先決條件。link:task-test-linux-system.html["了解如何檢查您的 Linux 主機是否已準備好安裝資料分類"]。
...
以私有模式安裝時，NetApp Data Classification 只能掃描同樣位於本地站台的資料來源中的資料。目前，NetApp Data Classification 可以掃描下列*本地*資料來源：
...
當 Data Classification 部署在 Console 代理程式上時，不支援將 Console 代理程式移轉至其他主機。如果您需要在實體隔離環境的不同 Console 代理程式上使用 Classification，請在新主機上部署新的 Console 代理程式和 Data Classification 執行個體。
...
* Data Classification 必須在專用主機上進行。該主機不能與其他應用程式或第三方軟體（例如防毒軟體）共享。
...
** *Azure VM 大小*："Standard_D16_v5"。link:reference-instance-types.html#azure-instance-types["查看其他 Azure 執行個體類型"^]。
...
* *Red Hat Subscription Management*：主機必須在 Red Hat Subscription Management 中註冊。如果未註冊，系統將無法存取儲存庫，從而無法在安裝期間更新所需的第三方軟體。
...
NOTE: 資料分類安裝程式會預留 UID 4321 和 GID 4321。在開始安裝之前，請確保主機上未使用 UID 4321 和 GID 4321。如果已使用，安裝將失敗並顯示「invalid user」錯誤訊息。
...
大多數情況下，資料分類功能安裝在單主機系統上。這與 Console 代理程式所在的虛擬機器主機不同。
