...
[role="lead"]
使用「私有模式」在本地站台的 Linux 主機上安裝 NetApp Data Classification，該主機無法存取網際網路。此部署使用安裝指令碼，並將 Data Classification 執行個體與 NetApp Console SaaS 階層中斷連線，讓您在隔離的環境中掃描本機資料來源。
